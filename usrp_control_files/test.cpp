// Modified UHD TXRX Loopback: optional TX-from-file support
// Combines the original txrx_loopback_to_file example with the
// send-from-file logic so you can transmit IQ samples from disk
// while still time-aligning RX to TX.
//
// New CLI flags:
//   --tx-file <path>        : If set, transmit samples from this file
//   --tx-type <short|float|double> : Sample type stored in --tx-file (default short=SC16)
//   --tx-spb <N>            : TX samples-per-buffer for file mode (0 => auto)
//   --tx-repeat             : Repeat file in a loop until Ctrl-C
//
// Build (inside UHD examples style project):
//   g++ -std=c++17 -O2 txrx_loopback_with_txfile.cpp -o txrx_loopback_with_txfile \
//      `pkg-config --cflags --libs uhd` -lboost_program_options -lboost_filesystem
//
// Example usage (baseband loopback on a BasicTX/RX with coax):
//   ./txrx_loopback_with_txfile \
//      --tx-rate 1e6 --rx-rate 1e6 --tx-freq 0 --rx-freq 0 \
//      --otw sc16 --type short --file rx_sc16.dat \
//      --tx-file iq_sc16.dat --tx-type short --nsamps 200000
//
// IQ file format expectations (per --tx-type):
//   short  => std::complex<int16_t>   (SC16: interleaved 16-bit I/Q)
//   float  => std::complex<float>     (FC32: interleaved 32-bit float I/Q)
//   double => std::complex<double>    (FC64: interleaved 64-bit float I/Q)
//
// Copyright headers preserved from original examples.
