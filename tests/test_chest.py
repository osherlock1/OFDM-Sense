...
def test_channel_estimation_unity_channel():
    ...
    gains = channel_estimation_calc(
        rx_pilot_freq=pilot_symbol, tx_pilot_ref=pilot_symbol, config=config
    )

    active_bins = np.unique(
        np.concatenate([config.data_carriers, config.pilot_carriers])
    )
...
def test_channel_estimation_known_gains():
    ...
    active_bines = np.unique(
        np.concatenate([config.data_carriers, config.pilot_carriers])
    )
    gains = channel_estimation_calc(
        rx_pilot_freq=pilot_symbol_rx, tx_pilot_ref=pilot_symbol_tx, config=config
    )
...
def test_apply_gains_unity_gains_uncahnged():
    ...
    applied_gains = apply_gains(
        rx_symb_freq=pilot_symbol_rx[active_bins], Lambda_est=gains
    )
