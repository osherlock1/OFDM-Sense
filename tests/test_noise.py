...
def test_add_noise_output_shape():
    ...
    noisy_signal = add_nosie(signal=pilot_symbol, snr_db=8)
...
def test_add_noise_reproducible_with_seed():
    config=OFDMConfig()
    pilot_symbol = preamble.generate_pilot_symbol(config=config)
    noisy_signal1 = add_nosie(signal=pilot_symbol, snr_db = 8, seed=42)
    noisy_signal2 = add_nosie(signal=pilot_symbol, snr_db = 8, seed=42)
    np.testing.assert_array_equal(noisy_signal1, noisy_signal2)

def test_add_noise_different_seeds_differ():
    config=OFDMConfig()
    pilot_symbol = preamble.generate_pilot_symbol(config=config)
    noisy_signal1 = add_nosie(signal=pilot_symbol, snr_db = 42, seed=42)
    noisy_signal2 = add_nosie(signal=pilot_symbol, snr_db = 8, seed=42)
...
def test_add_noise_snr_approximate():                                                                               
    ...
    target_snr_db = 30.0                                                                                            
    noisy_signal = add_nosie(signal=pilot_symbol, snr_db=target_snr_db, seed=42)                                    
