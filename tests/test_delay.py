import numpy as np
from ofdm.channel.delay import (
    matched_filter_calc,
    zero_padded_upsample,
    scale_rx_signal,
    calculate_sub_sample_delay_parabolic,
    calculate_sub_sample_delay_zp,
)
import pytest
...
class TestCalculateSubSampleDelayParabolic:
    ...
    def test_known_integer_delay(self):
        ...
        rx[20:30] = 1.0
        delay = calculate_sub_sample_delay_parabolic(
            rx_signal=rx, ref_signal=ref, fs=100e6
        )
        expected = 10 / 100e6
    ...
    def test_sub_sample_delay(self):
        ...
        rx = np.fft.ifft(
            np.fft.fft(ref) * np.exp(-1j * 2 * np.pi * freqs * true_delay_samples / fs)
        )
        delay = calculate_sub_sample_delay_parabolic(
            rx_signal=rx, ref_signal=ref, fs=fs
        )
...
class TestCalculateSubSampleDelayZeroPadded:
    ...
    def test_upsample_factor_affects_precision(self):
        ...
        delay_high = calculate_sub_sample_delay_zp(
            rx, ref, fs=100e6, upsample_factor=100
        )
