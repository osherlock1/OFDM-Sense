...
def main():
    ...
    parser.add_argument(
        "--n_symb", "-n", type=int, default=5, help="Number of Payload Symbols."
    )
    parser.add_argument("--snr", type=float, default=100.0, help="Add SNR in dB")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument(
        "--out", type=str, default="rand_ofdm_packet", help="Name of Output File"
    )
    ...
    for i in range(args.n_symb):
        ...
        # Map to Symbol
        data_symbol_freq = payload.generate_ofdm_data_symbol(
            config=ofdm_conf, iq_data=iq_data, pilot_data=pilot_vals
        )

        # Convert to time domain
        data_symbol_final = waveform.create_time_domain_symbol(
            data_symbol_freq, cp_len=ofdm_conf.CP_LEN
        )
    ...
    # Add noise
    if args.snr < 100.0:
        full_packet = noise.add_noise(
            signal=full_packet, snr_db=args.snr, seed=args.seed
        )
