...
def main():
    parser = argparse.ArgumentParser(
        description="Run TDOA multilateration on a processed experiment."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Device archive names e.g. RX3ch1 RX5ch1, RX7ch1",
    )
    parser.add_argument(
        "--anchor",
        type=str,
        required=True,
        help="Anchor device key in fixed_positions.json e.g ANCHORch0",
    )
    parser.add_argument(
        "--bias-ns",
        type=float,
        default=3.661,
        help="Hardware TDOA bias to subtract (nanoseconds)",
    )
    ...
    for device in args.devices:
        ...
        rover_positions.append(
            load_rover_position(archive_dir / "roaming_positions.json")
        )

        df = pd.read_csv(archive_dir / "delays.csv")
        device_dataframes.append(
            df[["run", "delay0", "delay1"]].rename(
                columns={"delay0": f"delay0_{device}", "delay1": f"delay1_{device}"}
            )
        )
    ...
    ax.set_title("OFDM Localization")
    plt.tight_layout()
    plt.show()
