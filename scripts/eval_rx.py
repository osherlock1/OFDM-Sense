...
def main():
    ...
    # Unpack Json Files
    rx_iq = np.array(rx_data["unpacked_data_real"]) + 1j * np.array(
        rx_data["unpacked_data_imag"]
    )
    ...
    sc_metrics = per_subcarrier_eval(
        rx_iq_data=rx_iq, ref_iq_data=ref_iq, config=ofdm_conf, n_sym=n_sym
    )


def per_subcarrier_eval(
    rx_iq_data: np.ndarray, ref_iq_data: np.ndarray, config: OFDMConfig, n_sym: int
):
    """
