...
def main():
    ...
    parser.add_argument(
        "--ref", action="store_true", help="Decalre if a directed wired ref is used"
    )
    ...
    if not args.file:
        for channel in rx_channel_idx:
            ...
            rx_data = scale_rx_signal(
                clean_rx(
                    np.fromfile(path, dtype=np.complex64),
                    start_idx_list[int(channel)][1],
                )
            )
            delay = (
        ...
        path = args.file
        rx_data = scale_rx_signal(
            clean_rx(np.fromfile(path, dtype=np.complex64), start_idx_list[int(0)][1])
        )
        calced_delays.append(
            calculate_sub_sample_delay_parabolic(
                rx_signal=rx_data, ref_signal=tx_pilot, fs=OFDM_CONF.FS
            )
        )

    json_data = {"delays": calced_delays, "mode": "no ref"}
