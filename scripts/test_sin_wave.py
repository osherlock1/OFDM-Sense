...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Single Tone Sine wave for Testing Hardware"
    )
    parser.add_argument(
        "--channel",
        "-c",
        type=str,
        help="Define the subdev specifications (A:0 B:0 for both, A:0 for left daughter boards, B:0 for right daughter boards)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2000,
        help="Specify number of samples in the generated tone",
    )
    parser.add_argument(
        "--tx_addr",
        type=str,
        help="IP address of TX USRP (need to do in form of addr=(ip))",
    )
    parser.add_argument(
        "--rx_addr", type=str, help="IP of RX USRP (need to do in form of addr=(ip))"
    )
    parser.add_argument(
        "--tx_channel_idx",
        type=str,
        help="Select chanenl for the TX (based on subdev specification), if no subdev was specified default = A:0 B:0",
    )
    parser.add_argument(
        "--rx_channel_idx",
        type=str,
    ...
    usrp.run_transfer(
        config=usrp_conf,
        tx_file=test_file_tx_path,
        rx_file=rx_file_path,
        nsamps=nsamps_final,
    )
    ...
    plotter.plot_symbol_freq(
        symbol=np.fft.fftshift(np.abs(np.fft.fft(signal))),
        title=f"Rx FFT Plot Freq = {FREQ}",
    )

    # Plot Ref data
    plotter.plot_time_series(signal=ref_signal, fs=FS, title="Ref Sine Wave Real")
    plotter.plot_symbol_freq(
        symbol=np.fft.fftshift(np.abs(np.fft.fft(ref_signal))), title="Ref FFT Plot"
    )
    plt.show()
...
def generate_tone(fs: float, freq: float, n_samples: float, n_buffer: int = 1000):
    ...
    bin_path = "data_files/test_sin.dat"
    ...
    json_path = "data_files/test_sin_ref.json"
