...
def _synchronize(rx_raw: np.ndarray, ref_data: dict, ofdm_conf: OFDMConfig) -> int:
    sync_ref = (
        np.array(ref_data["sync_ref_real"]).astype(complex)
        + 1j * np.array(ref_data["sync_ref_imag"]).astype(complex)
    )
...
def _estimate_and_correct_cfo(
    ...
    pilot_search_area = rx_raw[
        coarse_pilot_start - search_margin : coarse_pilot_start + sym_len + search_margin
    ]

    tx_pilot_ref = (
        np.array(ref_data["pilot_ref_real"]).astype(complex)
        + 1j * np.array(ref_data["pilot_ref_imag"]).astype(complex)
    )
...
def _demodulate_payload(
    ...
    tx_pilot_freq = waveform.time_to_freq(waveform.remove_cp(tx_pilot_ref, cp_len=ofdm_conf.CP_LEN))
    rx_pilot_freq = waveform.time_to_freq(waveform.remove_cp(rx_pilot_sym, cp_len=ofdm_conf.CP_LEN))
    ...
    for sym_time in rx_payload_syms:
        sym_freq = waveform.time_to_freq(waveform.remove_cp(sym_time, cp_len=ofdm_conf.CP_LEN))
