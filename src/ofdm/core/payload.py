...
def generate_ofdm_data_symbol(
    config: OFDMConfig, iq_data: np.ndarray, pilot_data: np.ndarray
) -> np.ndarray:
    """
    ...
    if len(iq_data) != len(config.data_carriers):
        raise ValueError(
            f"Lenth of Iq Data:{len(iq_data)} does not match length of Data Carrier Mapping: {len(config.data_carriers)} "
        )
    if len(pilot_data) != len(config.pilot_carriers):
        raise ValueError(
            f"Length of Pilot Data:{len(pilot_data)} does not match length of Pilot Carrier mapping {len(config.pilot_carriers)}"
        )

    # Add IQ Data
    symbol_freq[np.array(config.data_carriers)] = iq_data

    # Add Pilot Data
    symbol_freq[np.array(config.pilot_carriers)] = pilot_data
    return symbol_freq
